...
module bottle_nft::bottle_nft {
    ...
    public struct BottleMinted has copy, drop {
        bottle_id: ID,
        name: String,
        vintage: u64,
        owner: address,
    ...
    public struct BottleTransferred has copy, drop {
        bottle_id: ID,
        from: address,
        to: address,
        timestamp: u64
    ...
    public struct BottleVerified has copy, drop {
        bottle_id: ID,
        qr_code: String,
        verifier: address,
        timestamp: u64
    ...
    public struct ProvenanceAdded has copy, drop {
        bottle_id: ID,
        event_type: String,
        timestamp: u64
    }
    ...
    public entry fun mint_bottle(
        ...
        let mut custom_text_mut = custom_text;
        let custom_text_string = if (option::is_some(&custom_text_mut)) {
            option::some(string::utf8(option::extract(&mut custom_text_mut)))
        } else {
            option::none()
        };

        // Create initial provenance entry
        let mut provenance_events = vec_map::empty();
        vec_map::insert(
    ...
    /// Note: Provenance events should be added separately using add_provenance_event
    public entry fun transfer_bottle(
        ...
        // For now, just transfer - provenance can be added by new owner
        // In a production version, we'd need to use a helper function or
        // different data structure to update VecMap during transfer
        ...
        // Transfer using standard Sui transfer (owner field will be updated automatically)
        transfer::public_transfer(bottle, recipient);
    }
